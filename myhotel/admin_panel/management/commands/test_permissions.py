...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                f"Total permissions: {len(context['user_permission_codenames'])}"
            )
            self.stdout.write(
                f"Permission codenames: {context['user_permission_codenames']}"
            )
            ...
            if user.is_superuser:
                ...
                if can_view_bookings and not can_edit_bookings:
                    self.stdout.write(
                        "✓ Read-only access to bookings working correctly"
                    )
