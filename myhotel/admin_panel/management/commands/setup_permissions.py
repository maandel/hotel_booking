    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            booking_permissions = Permission.objects.filter(
                codename__in=["view_booking", "edit_booking", "view_contactform"]
            )
        ...
        guest_manager_role, created = Role.objects.get_or_create(
            name="Guest Manager",
            defaults={"description": "Can manage guest information", "is_active": True},
        )

        if created:
            # Add permissions for guest manager
            guest_permissions = Permission.objects.filter(
                codename__in=["view_guest", "add_guest", "edit_guest", "delete_guest"]
            )
        ...
        for role in Role.objects.filter(is_active=True):
            self.stdout.write(
                f"  - {role.name}: {role.permissions.count()} permissions"
            )
