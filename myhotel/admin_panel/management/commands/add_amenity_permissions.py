...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            "Adding hotel amenities and room amenities section access permissions..."
        )
        ...
        generic_ct, _ = ContentType.objects.get_or_create(
            app_label="admin_panel", model="section_access"
        )
        ...
        for codename, name, description in new_permissions:
            ...
            if Permission.objects.filter(codename=codename).exists():
                self.stdout.write(
                    f"  ⚠️ Permission {codename} already exists, skipping..."
                )
                continue
            ...
            try:
                ...
                try:
                    ...
                    created_count += 1
                    self.stdout.write(
                        f"  ✓ Created: {permission.name} (with add permission_type)"
                    )
                except IntegrityError:
                    self.stdout.write(
                        self.style.ERROR(
                            f"  ❌ Could not create permission {codename} due to integrity constraints"
                        )
                    )
