...
from hotel.models import Permission, UserRole
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            if has_section_access(request.user, section):
                ...
                messages.error(
                    request, f"You do not have access to the {section} section."
                )
                return redirect(redirect_url)
...
def require_model_permission(
    model_class, permission_type, redirect_url="admin_panel:dashboard"
):
    """
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if has_permission(request.user, permission_codename):
                ...
                messages.error(
                    request,
                    f"You do not have permission to {permission_type} {content_type.model} records.",
                )
                return redirect(redirect_url)
...
class PermissionMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.required_section and not has_section_access(
            request.user, self.required_section
        ):
            messages.error(
                request,
                f"You do not have access to the {self.required_section} section.",
            )
        ...
        if self.permission_required:
            if not has_permission(request.user, self.permission_required):
                messages.error(
                    request,
                    f"You do not have permission to access this page. Required permission: {self.permission_required}",
                )
                return redirect(self.redirect_url)
        ...
        # Check model permission
            ...
            if not has_permission(request.user, permission_codename):
                messages.error(
                    request,
                    f"You do not have permission to {self.model_permission_type} {content_type.model} records.",
                )
                return redirect(self.redirect_url)
