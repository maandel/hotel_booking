...
def admin_bookings(request):
    ...
    if search:
        ...
        bookings = bookings.filter(
            Q(id__in=guest_bookings) | Q(room__room_number__icontains=search)
        )
...
def admin_booking_view(request, booking_id):
    ...
    return render(
        request, "admin_panel/booking_view.html", {"booking": booking, "guests": guests}
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if new_days > original_days:
                messages.error(
                    request,
                    f"Cannot extend booking beyond {original_days} days already paid for. New booking duration is {new_days} days.",
                )
                return redirect(
                    reverse("admin_panel:admin_booking_edit", args=[booking_id])
                )
...
def api_check_room_availability(request):
    ...
    try:
        ...
        if checkin_date >= checkout_date:
            return JsonResponse(
                {"available": False, "error": "Check-out must be after check-in."}
            )
        ...
        available = is_room_available(
            room, checkin_date, checkout_date, exclude_booking_id=booking_id
        )
