        ...
        try:
            if permission_category == "model":
                ...
                if not content_type_id or not permission_type:
                    messages.error(
                        request,
                        "Content type and permission type are required for model permissions.",
                    )
                ...
                if Permission.objects.filter(
                    content_type=content_type, permission_type=permission_type
                ).exists():
                    messages.error(
                        request,
                        f'Permission "{permission_type}" for "{content_type.model}" already exists.',
                    )
                ...
                if not section_name:
                    messages.error(
                        request,
                        "Section name is required for section access permissions.",
                    )
                    return redirect("admin_permission_create")
            ...
            messages.success(
                request, f'Permission "{permission.name}" created successfully.'
            )
            return redirect("admin_permissions")

        except Exception as e:
            messages.error(request, f"Error creating permission: {e!s}")
...
def admin_permission_edit(request, permission_id):
    ...
    if request.method == "POST":
        ...
        try:
            permission.save()
            messages.success(
                request, f'Permission "{permission.name}" updated successfully.'
            )
            return redirect("admin_permissions")
        except Exception as e:
            messages.error(request, f"Error updating permission: {e!s}")
...
def admin_permission_delete(request, permission_id):
    ...
    if request.method == "POST":
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            try:
                ...
                return JsonResponse(
                    {
                        "success": False,
                        "message": f"Error deleting permission: {e!s}",
                    },
                    status=400,
                )
        ...
        permission.delete()
        messages.success(
            request, f'Permission "{permission_name}" deleted successfully.'
        )
        return redirect("admin_permissions")
...
def admin_role_create(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, f"Error creating role: {e!s}")

    # Get all permissions grouped by content type
    permissions = Permission.objects.select_related("content_type").order_by(
        "content_type__model", "permission_type"
    )

    # Separate section access permissions for better organization
    section_access_permissions = Permission.objects.filter(
        codename__startswith="access_"
    )
    regular_permissions = permissions.exclude(
        id__in=section_access_permissions.values_list("id", flat=True)
    )
...
def admin_role_edit(request, role_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, f"Error updating role: {e!s}")
    ...
    if request.method == "POST":
        ...
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            try:
                ...
                return JsonResponse(
                    {"success": False, "message": f"Error deleting role: {e!s}"},
                    status=400,
                )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if UserRole.objects.filter(user=user, role=role).exists():
                messages.error(
                    request, f'User "{user.username}" already has role "{role.name}".'
                )
            ...
            messages.success(
                request,
                f'Role "{role.name}" assigned to user "{user.username}" successfully.',
            )
            return redirect("admin_user_roles")

        except Exception as e:
            messages.error(request, f"Error assigning role: {e!s}")
...
def admin_user_role_remove(request, user_role_id):
    ...
    if request.method == "POST":
        ...
        user_role.delete()
        messages.success(
            request, f'Role "{role_name}" removed from user "{user_name}" successfully.'
        )
        return redirect("admin_user_roles")
...
def create_default_permissions(request):
    ...
    try:
        ...
        if created_count > 0:
            messages.success(
                request, f"Created {created_count} default permissions successfully."
            )
        else:
            messages.info(request, "All default permissions already exist.")

    except Exception as e:
        messages.error(request, f"Error creating default permissions: {e!s}")
