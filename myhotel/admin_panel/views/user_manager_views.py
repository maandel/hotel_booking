...
from ..forms import AdminUserEditForm, AdminUserRegistrationForm, UserSearchForm
...
def user_manager_list(request):
    ...
    if form.is_valid():
        ...
        if role:
            user_ids = UserRole.objects.filter(role=role).values_list(
                "user_id", flat=True
            )
        ...
        if role_id:
            try:
                ...
                if UserRole.objects.filter(user=user, role=role).exists():
                    messages.warning(
                        request,
                        f'User "{user.username}" already has role "{role.name}".',
                    )
                else:
...
def user_manager_remove_role(request, user_id, role_id):
    ...
    try:
        ...
        messages.success(
            request,
            f'Role "{role.name}" removed from user "{user.username}" successfully.',
        )
...
def user_manager_stats(request):
    ...
    for role in Role.objects.filter(is_active=True):
        ...
        role_stats.append({"role": role.name, "user_count": user_count})
...
def user_manager_generate_password(request, user_id):
    ...
    messages.success(
        request, f'New password generated for user "{user.username}": {new_password}'
    )
    messages.warning(
        request,
        "Please share this password securely with the user and ask them to change it on first login.",
    )
