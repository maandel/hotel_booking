...
class RoomAvailabilityChecker:
    ...
    def check_availability(
        self, checkin_str, checkout_str, room_type_id, requested_rooms, guest
    ):
        self.form_data = {
            "checkin": checkin_str,
        ...
        if not all([checkin_str, checkout_str, room_type_id, requested_rooms]):
            self.errors.append(
                "All fields (check-in, check-out, room type, number of rooms) are required."
            )
        ...
        try:
            ...
            self.errors.append(f"Unexpected error checking availability: {e!s}")
            logger.error(f"Unexpected error in check_availability: {e!s}")
            return {"errors": self.errors, "form_data": self.form_data}
