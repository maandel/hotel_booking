...
def submit_booking(request):
    if request.method != "POST":
        ...
        return JsonResponse(
            {"success": False, "errors": "Invalid request method"}, status=400
        )
    ...
    try:
        ...
        base_price_str = (
            request.POST.get("modalBasePrice", "0").replace("₦", "").strip()
        )
        total_cost_str = (
            request.POST.get("modalTotalCost", "0").replace("₦", "").strip()
        )
        ...
        logger.error(
            f"transaction_id: {request.POST.get('transaction_id')}, payment_status: {request.POST.get('payment_status')}"
        )
        ...
        if missing:
            ...
            return JsonResponse(
                {"success": False, "errors": f"Missing fields: {', '.join(missing)}"},
                status=400,
            )
        ...
        if checkin >= checkout or checkin < date.today():
            ...
            return JsonResponse(
                {"success": False, "errors": "Invalid check-in/check-out dates"},
                status=400,
            )
        if guests <= 0 or rooms <= 0:
            logger.warning("Guests and rooms must be positive")
            return JsonResponse(
                {"success": False, "errors": "Guests and rooms must be positive"},
                status=400,
            )

        Decimal(base_price_str)
        ...
        if available_rooms.count() < rooms:
            logger.warning(
                f"Not enough rooms available: {rooms} requested, {available_rooms.count()} available"
            )
            return JsonResponse(
                {
                    "success": False,
                    "errors": f"Only {available_rooms.count()} room(s) available",
                },
                status=400,
            )

        # Accept payment_status and transaction_id from POST (preferred over session)
        payment_status = request.POST.get("payment_status") or request.session.get(
            "payment_status"
        )
        transaction_id = request.POST.get("transaction_id") or request.session.get(
            "transaction_id"
        )
        ...
        if payment_status != "success" or not transaction_id or not expected_amount:
            ...
            return JsonResponse(
                {"success": False, "errors": "Payment not verified"}, status=400
            )
        ...
        if existing_guest:
            ...
            if existing_guest_by_phone:
                ...
                guest.save()
                logger.info(
                    f"Existing guest found by phone and updated: {phone} -> {email}"
                )
            else:
        ...
        logger.info(
            f"Booking created: ID {booking.id} for {guest.email}, transaction_id={transaction_id}"
        )
        ...
        return JsonResponse(
            {"success": False, "errors": "Invalid room type"}, status=400
        )
    except ValidationError as e:
        logger.error(f"Validation error: {e!s}")
        return JsonResponse({"success": False, "errors": str(e)}, status=400)
    except ValueError as e:
        logger.error(f"Invalid numeric value: {e!s}")
        return JsonResponse(
            {"success": False, "errors": f"Invalid numeric value: {e!s}"}, status=400
        )
    except Exception as e:
        logger.error(f"Unexpected error: {e!s}")
        return JsonResponse(
            {"success": False, "errors": f"Unexpected error: {e!s}"}, status=400
        )
...
def verify_payment(request):
    try:
        ...
        logger.error("=== VERIFY PAYMENT START ===")
        ...
        if hasattr(request, "body") and request.body:
            try:
                ...
                logger.error(f"Error parsing request body: {e!s}")
        ...
        if not reference:
            ...
            return JsonResponse(
                {"status": "error", "message": "Missing reference"}, status=400
            )

        # Check if Paystack secret key is configured
        if (
            not hasattr(settings, "PAYSTACK_SECRET_KEY")
        ...
        logger.error(
            f"Sending verification request to Paystack for reference: {reference}"
        )
        ...
        try:
            ...
            logger.error(f"Network error verifying payment: {e!s}")
            return JsonResponse(
                {"status": "error", "message": f"Network error: {e!s}"}, status=500
            )
        ...
        logger.error(
            f"Paystack response: status_code={response.status_code}, data={response_data}"
        )
        ...
        if (
            ...
            paid_amount = (
                Decimal(response_data["data"]["amount"]) / 100
            )  # Convert kobo to Naira
            logger.error(
                f"Comparing amounts: expected={expected_amount}, paid={paid_amount}, raw_paid_amount={response_data['data']['amount']}"
            )

            if abs(paid_amount - expected_amount) > Decimal(
                "0.01"
            ):  # Allow small rounding differences
                logger.error(
                    f"Amount mismatch: expected {expected_amount}, paid {paid_amount}"
                )
                return JsonResponse(
                    {
                        "status": "error",
            ...
            logger.info(
                f"Payment verified: reference={reference}, transaction_id={response_data['data']['id']}"
            )
            return JsonResponse(
                {
                    "status": "success",
            ...
            logger.error(
                f"Payment verification failed: reference={reference}, response: {response_data}"
            )
            error_message = "Payment verification failed"
            if response_data.get("message"):
                error_message = response_data["message"]
            return JsonResponse(
                {"status": "failed", "message": error_message}, status=400
            )

    except Exception as e:
        logger.error(f"Unexpected error verifying payment: {e!s}", exc_info=True)
        return JsonResponse(
            {"status": "error", "message": f"Unexpected error: {e!s}"}, status=500
        )
...
def store_expected_amount(request):
    try:
        ...
        if not amount or float(amount) <= 0:
            ...
            return JsonResponse(
                {"success": False, "errors": "Invalid amount"}, status=400
            )
        request.session["expected_amount"] = str(amount)
        logger.info(f"Stored expected amount: {amount}")
        return JsonResponse({"success": True})
    except Exception as e:
        logger.error(f"Error storing expected amount: {e!s}")
        return JsonResponse({"success": False, "errors": str(e)}, status=400)
...
def webhook(request):
    try:
        ...
        if event == "charge.success":
            ...
            logger.info(
                f"Webhook received: event={event}, reference={reference}, status={status}"
            )
