...
from .models import Hotel, HotelAmenity, RoomType
...
def home(request):
    ...
    if request.method == "POST" and "form1_submit" in request.POST:
        ...
        result = checker.check_availability(
            checkin_str, checkout_str, room_type_id, requested_rooms, guest
        )
...
def check_availability_ajax(request):
    ...
    try:
        ...
        logger.info(
            f"Parsed parameters: checkin={checkin_str}, checkout={checkout_str}, "
        ...
        logger.error(f"Error in check_availability_ajax: {e!s}", exc_info=True)
        return JsonResponse(
            {"errors": [f"Server error: {e!s}"], "success": False}, status=500
        )
